...
export default function SideMenu() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (!menuInScope) {
        // find min
        setCurrentMenu(
          menus[distanceArr.indexOf(Math.min(...distanceArr))].anchor,
        );
        return;
      } else {
  ...
  return (
    <div className="fixed left-0 sidemenu:left-4 top-1/3 flex-col gap-2 font-mono text-lg items-end text-gray-500 w-[200px] z-10 hidden sidemenu:flex">
      {menus.map((menu) => (
        <span
          key={menu.label}
